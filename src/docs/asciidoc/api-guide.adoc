...
Menu Generation RESTful API also defines an https://swagger.io/specification/[Open API] specification. This specification is part of the incoming requests validation, and should be used by clients to consume the API.
...
A `POST` request will create a recipe. Only an authenticated user can create recipes. The user that creates a recipe is
called a recipe author.
...
A `DELETE` request will delete the recipe. Only the recipe author or a super administrator can delete a recipe.
...
A `POST` request will add an ingredient to a recipe's related ingredients collection. Only the recipe author or a super
administrator can add an ingredient to a recipe.
...
<<measurement-types-list, measurement type>>. Note that a measurement unit won't be accepted during recipe ingredient
creation if its associated type does not belong to the ingredient's measurement types.
...
A `DELETE` request will delete all of the recipe's related ingredients. Only the recipe author or a super administrator
can delete all recipe's ingredients directly.
...
A `DELETE` request will delete the recipe ingredient. Only the recipe author or a super administrator can remove
